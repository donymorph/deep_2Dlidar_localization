...
########################################
# Candidate Mamba-based Model: MambaNet
########################################
class MambaNet(nn.Module):
    ...
    The model embeds each beam, adds positional encoding, and then passes the
    resulting sequence through a stack of Mamba blocks. Global average pooling then
    produces the final regression outputs: (pos_x, pos_y, orientation_z).
    ...
    def forward(self, x):
        ...
        # Pass through each Mamba block with residual connection.
        for block in self.blocks:
            x_embed = x_embed + block(x_embed)
        
        # Pooling: rearrange to (B, d_model, input_size) and pool over beams.
...
def train_and_eval(model, train_loader, val_loader, device, trial, max_epochs=10):
    ...
    for epoch in range(max_epochs):
        ...
        # Use only the first two coordinates (x,y) for accuracy calculation.
        accuracy, x_thresh, y_thresh = calc_accuracy_percentage_xy(all_gts[:, :2], all_preds[:, :2])
        ...
        #logger.info(f"Epoch {epoch}: Val Loss: {val_loss:.4f}, Accuracy: {accuracy:.1f}% (thresholds: {x_thresh}, {y_thresh})")
...
if __name__ == "__main__":
    ...
    storage_url = "sqlite:///mamba_search.db"
    study_name  = "mamba_search"
    ...
    logger.info("Run: optuna-dashboard sqlite:///mamba_search.db")
